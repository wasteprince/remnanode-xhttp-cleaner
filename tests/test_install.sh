...
grep -q 'clean_established_outbound.*False' "$ROOT_DIR/install.sh"
grep -q 'clean_xhttp_buffers.*False' "$ROOT_DIR/install.sh"
grep -q 'core_memory_optimizer.go' "$ROOT_DIR/install.sh"
grep -q 'hysteria_memory_guard.go' "$ROOT_DIR/install.sh"
