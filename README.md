...
После завершения сессии Go не обязан сразу вернуть освобождённые страницы ОС. Optimizer последовательно вызывает `runtime.GC()` и `debug.FreeOSMemory()` не чаще одного раза в пять минут и только при runtime footprint не ниже `max(256 MiB, memory ceiling / 16)`. Отдельный GC в XHTTP reaper не запускается.
...
| `/var/cache/remnanode-xhttp-clean/` | Проверенные artifacts и повторно используемые Go modules |
