...
> Безопасный очиститель старых TCP-сокетов Xray — **by Bankaev**.
...
Замените `YOUR_GITHUB_USERNAME` на владельца репозитория:

```bash
sudo mkdir -p /opt/node-xhttp
cd /opt/node-xhttp
sudo git clone https://github.com/YOUR_GITHUB_USERNAME/node-xhttp.git .
```
...
После установки доступна интерактивная панель **XHTTP Cleaner by Bankaev**:
